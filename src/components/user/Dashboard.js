...
//       alert("Please login again and then delete your account.");
//     } else {
...
function Dashboard() {
  ...
  // ✅ Auth state - added [] dependency to avoid infinite loop
  useEffect(() => {
        ...
        <div className="row justify-content-center gap-2">
          ...
          <div className="col-12 col-lg-5 card p-1 p-lg-2">
            ...
            <div className="container-fluid mt-4">
              <div className="row justify-content-center gap-1 gap-lg-2">
                <div className="col-4 col-lg-3 card align-items-center bg-primary text-light">
                  <h2 className="m-0">{myAnswers.length}</h2>
                  <p style={{ fontWeight: "350" }}>Answers</p>
                </div>
                <div className="col-3 col-lg-3 card align-items-center bg-success text-light">
                  <h2 className="m-0">{myQuestions.length}</h2>
                  <p style={{ fontWeight: "350" }}>Questions</p>
                </div>
                <div className="col-4 col-lg-3 card align-items-center bg-warning text-light">
                  <h2 className="m-0">
                    {votesAData.length + votesQData.length}
                  </h2>
                  <p style={{ fontWeight: "350" }}>Votes cast</p>
                </div>
              </div>
            </div>
            ...
            <div className="d-flex justify-content-center gap-3">
              ...
              <button
                className="btn btn-sm btn-outline-danger mt-5 w-25"
                onClick={handleDeleteAccount}
              >
                Delete Account
              </button>
          ...
          {/* ✅ Fixed: myQuestions.length (not .lenght) */}
          <BadgeComponent
            no_of_questions_answered={Number(myAnswers.length)}
            votes_gained={Number(votesGainedA)}
            no_of_questions_asked={Number(myQuestions.length)}
            votes_gained_for_questions={Number(votesGainedQ)}
          />
        </div>
      </div>
      ...
      <div className="container">
        ...
        <div className="row p-2 card mb-5">
          <div>
            ...
            <ul className="list-group ms-5">
              {ansQueVoteQMerge.map((child, index) => (
                <li
                  key={index}
                  className="list-group-item w-75 mb-3 d-flex justify-content-between align-items-start"
                >
                  <div className="ms-2 me-auto">
                    <div className="fw-light">{child.title}</div>
                  </div>
            ...
            <ul className="list-group ms-5">
              {ansQueVoteAMerge.map((child, index) => (
                <li
                  key={index}
                  className="list-group-item w-75 mb-3 d-flex justify-content-between align-items-start"
                >
...
// ✅ Fixed BadgeComponent - correct totalPoints calculation
const BadgeComponent = (props) => {
  const [totalPoint, setTotalPoints] = useState(0);
  ...
  useEffect(() => {
    const totalPoints =
      props.no_of_questions_answered * 1 +
      props.votes_gained * 2 +
      props.no_of_questions_asked * 1 +
      props.votes_gained_for_questions * 3;
    ...
    props.no_of_questions_answered,
    props.votes_gained,
    props.no_of_questions_asked,
    props.votes_gained_for_questions,
  ]);

  return (
    <div className="col-12 col-lg-3 card text-center">
      <h4 className="mt-5">Total Points</h4>
      <p style={{ fontSize: "2rem", fontWeight: "bold" }}>{totalPoint}</p>
      <p>{badge}</p>
      <lottie-player
        src="https://lottie.host/83058c04-2752-4e2c-a2b8-a3f78d215498/QK3klvQLzQ.json"
        background="#FFFFFF"
